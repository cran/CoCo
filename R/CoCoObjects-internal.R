...
function (model, data = NULL, object = .currentCoCo(), names = NULL, 
...
".get.env.CoCoVIEWS" <-
function (id = CoCoVIEWS@.id.env, CoCoVIEWS = NULL, 
    env = .get.env.CoCoModelOBJECT(id = id.fv, 
    env = env.fm)$env, 
    id.fv = if (is.null(CoCoModelOBJECT)) 
        .but.last(id) else CoCoModelOBJECT@.id.env, 
    CoCoModelOBJECT = NULL, env.fm = .get.env.CoCoOBJECT(id = id.fm, 
        env = .CoCo.Root$env)$env, 
    id.fm = if (is.null(CoCoOBJECT)) 
        .but.last(.but.last(id)) else CoCoOBJECT@.id.env, 
    CoCoOBJECT = NULL) 
{
    if (is.null(env)) {
        if (message) {
...
".new.coco" <-
function (object = .currentCoCo(), type = 1, uniq.title = FALSE, 
    title = "A CoCo object", parent = .CoCo.Root, 
    env = .CoCo.toplevel(parent)) 
{
    result <- object
    ...
    my.assign(".instances.coco", c(.instancesCoCo(), result@.reference), 
        frame = 0)
    return(result)
}
".new.coco.model" <-
function (number, model, object = .currentCoCo(), title = "", 
    key = .return.key(object = object), 
    identification = .return.reference(object = object), 
    id.env = .return.id.env(object), 
    parent.id.env = if (class(object) == "CoCoModelClass") 
        .but.last(id.env) else id.env, 
    parent = .get.env.CoCoOBJECT(id = parent.id.env), 
    env = .CoCo.toplevel(parent, key = key, reference = identification, 
        number = number)) 
{
    result <- number
    result <- new("CoCoModelClass", 
        .reference = .return.reference(object = object), 
    ...
    if (ifelse(is.character(title), title != "", title)) 
        result@.title <- title
    my.assign(".instances.coco.models", c(.instancesCoCoModels(), 
    ...
    levels = NULL, to.factor = NULL, discrete = NULL, continuous = NULL) 
{
    ...
    if (.is.nil.model(model)) 
        ...
        enterTwoLists(discrete = levels[levels == 1], 
            continuous = levels[levels != 
            1])
        enterModel("*")
    }
...
".object.of.thing" <-
function (data = NULL, object = .currentCoCo(), to.factor = NULL) 
{
    result <- object
...
function (object = .currentCoCo(), recover = FALSE) 
{
    ...
    if ((identification == .endedCoCo()) && recover) {
    ...
    if (is.null(result) || (identification == .endedCoCo())) 
        stop("Recovering ended CoCo object!!!")
    if (identification == 0 || any(.CoCoIdentifications()[, 1] == 
        identification)) 
        return(result)
...
function (coco.model.object) 
{
    ...
    .my.trace(".recover.model,          start:", level = 10000, 
        name = "xxx", key = .return.key(coco.model.object), 
    ...
    .my.trace(".recover.model,           stop:", level = 90000, 
        ...
        model = .return.model.of.object(coco.model.object), 
        id = .return.reference(coco.model.object), 
        number = .return.model.number(coco.model.object), 
...
".return.model.number" <-
function (coco.model = .currentCoCo()) 
{
...
function (model, object = .currentCoCo()) 
{
...
function (number, recover = TRUE, object = .currentCoCo()) 
{
