"set.ic.kappa" <-
function (kappa = 2, object = .currentCoCo()) 
{
    if (kappa == "what")
    ...
    data = NULL, object = .object.of.thing(data = data)) 
{
    ".option.long" <- function(result, names, names.both, names.args, 
        args, code, m = length(names), object = .currentCoCo()) {
    ...
    ".option.longs" <- function(result, names, names.both, names.args, 
        args, code, m = 25, stop.mark = 0, object = .currentCoCo()) {
        if (length(intersect(names.both, names)) > 0) {
            X <- call.coco.longs(code, "what", m, FALSE, object = object)
    ...
    ".option.long.and.real" <- function(result, long.names, real.names, 
        ...
        real.m = length(real.names), object = .currentCoCo(),
        sub.code = 1) {
    ...
    ".option.code" <- function(result, name, names.both, names.args, 
        args, code, x = c("a", "b", "c"), y = 1:3, object = .currentCoCo()) {
        if (length(intersect(names.both, name)) > 0) {
    ...
    ".option.switch" <- function(result, name, names.both, names.args, 
        args, switch.name = name, object = .currentCoCo()) {
        if (length(intersect(names.both, name)) > 0) {
    ...
    "options.formats" <- function(..., status = FALSE, object = .currentCoCo()) {
        if (status) 
    ...
    "options.tests" <- function(..., status = FALSE, object = .currentCoCo()) {
        if (status) 
    ...
    "options.ips" <- function(..., status = FALSE, object = .currentCoCo()) {
        if (status) 
    ...
    "options.cips" <- function(..., status = FALSE, object = .currentCoCo()) {
        if (status) 
            showOptions("ips", object = object)
        args <- list(...)
        if ((length(args) == 1) && is.list(args[[1]])) 
    ...
    "options.em" <- function(..., status = FALSE, object = .currentCoCo()) {
        if (status) 
    ...
    "options.mixed" <- function(..., status = FALSE, object = .currentCoCo()) {
        if (status) 
    ...
    "options.exact" <- function(..., status = FALSE, object = .currentCoCo()) {
        if (status) 
    ...
    "options.other" <- function(..., status = FALSE, object = .currentCoCo()) {
        if (status) 
    ...
    "options.files" <- function(..., status = FALSE, object = .currentCoCo()) {
        if (status) 
    ...
    "options.datafiles" <- function(..., status = FALSE, object = .currentCoCo()) {
        if (status) 
