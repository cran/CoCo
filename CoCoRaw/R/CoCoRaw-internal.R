...
function (set = "", model.1 = "current", model.2 = "base", data = NULL, 
    object = .object.of.models(model.1, model.2, data = data, 
        ...), ...) 
{
    coco.string.double(158, set, FALSE, model.1, model.2, type = "unconditioned", 
        object = object)
}
    ...
    object = .object.of.thing(data = data, ...), ...) 
{
    ...
    if (FALSE) 
        ...
        result <- call.coco(105, sub.code = ifelse(accumulated, 
            2, 1), arg.char = "", arg.long = arg.long, arg.double = arg.double, 
...
function (order = 1, set = "*", model = FALSE, data = NULL, object = .object.of.model(model, 
    data = data, ...), ...) 
{
...
function (model) 
{
    ...
    else names <- sort(unique(unlist(.split.model.gc(model, split.generators = TRUE))))
    nv <- c(",", ".", ";", "^")
        ...
        data = data, ...), ...) 
{
    ...
    result <- call.coco(116, 1, arg.char = set, arg.long = c(sum(c(.table.value(x), 
    ...
    object = .object.of.model(model, data = data, ...), ...) 
{
    ".split.junction.tree" <- function(tree, split.sets = FALSE, 
        split.models = FALSE, split.generators = FALSE, eliminate.empty = TRUE) {
            ...
            if (any(chars == "{")) {
                ...
                if (any(chars == "<")) {
                  ...
                  return(list(left = .split.junction.tree(left, 
                    ...
                    split.generators = split.generators, eliminate.empty = eliminate.empty), 
                  ...
                  if (any(chars == ",")) {
                    ...
                    if (split.models) 
                      model <- .split.model.gc(model, split.generators = split.generators)
                    ...
                    else return(.split.set.and.flags(tree, split.sets = split.sets))
                  }
                }
            }
            else return(tree)
...
".set.datastructure" <-
function (code = "all", setslot = TRUE, data = NULL, object = .object.of.thing(data = data, 
    ...), ...) 
{
    if (("what" != code) && setslot) 
        .set.coco.value(object, ".medio", append = TRUE, list(type = "set.datastructure", 
            code = code))
    ...
    if (n > 0) {
        ...
        if (any(chars == "|")) {
            ...
            if (length(k) > 0) {
                ...
                structure <- .split.model.gc(structure, split.generators = split.generators, 
            ...
            if (length(k) == 1) {
                ...
                model <- .split.model.gc(model, split.generators = split.generators, 
                  eliminate.empty = eliminate.empty)
                options <- .split.model.gc(options, split.generators = split.generators, 
                  eliminate.empty = eliminate.empty)
                ...
                if (length(k) == 2) 
                  ...
                  rest <- .split.model.gc(rest, split.generators = split.generators, 
                    eliminate.empty = eliminate.empty)
                }
                discrete <- .split.model.gc(discrete, split.generators = split.generators, 
                  eliminate.empty = eliminate.empty)
                linear <- .split.model.gc(linear, split.generators = split.generators, 
                  eliminate.empty = eliminate.empty)
                quadratic <- .split.model.gc(quadratic, split.generators = split.generators, 
                  eliminate.empty = eliminate.empty)
                if (length(k) == 2) 
