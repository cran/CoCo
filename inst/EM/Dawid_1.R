...
optionsCoCo(diary.name="dawid1.dia", 
            force.files = TRUE, object = CoCo.Object);
optionsCoCo(report.name="dawid1.rep", 
            force.files = TRUE, object = CoCo.Object);
optionsCoCo(timer  = TRUE, object = CoCo.Object);

levels <- apply(Dawid79, 2, max)[-1];
missing <- rep(0, ncol(Dawid79) - 1);
...
# You can ignore the warning on "File as datastructure ... " under Linux/Unix.
enterList(c(t(Dawid79)), accumulated = TRUE, object = CoCo.Object);
showTable(output.form = "sparse.table")

# Exclude "v" and "w":

Dawid79.vw <- Dawid79[,-(2:3)]

levels <- apply(Dawid79.vw, 2, max)[-1];
missing <- rep(0, ncol(Dawid79.vw) - 1);
names(missing) <- names(levels)
missing["x"] <- 1
result <- enterNames(names(levels), levels - missing, missing = missing,
                          object = CoCo.Object);
...
enterList(c(t(Dawid79.vw)), accumulated = TRUE, object = CoCo.Object);
...
enterModel("ax,bx,cx,2x,3x,4x,5x", object = CoCo.Object);
showDeviance(object = CoCo.Object);

.show.log.lik(object = CoCo.Object);

showTable("probabilities", "ax", object = CoCo.Object);
showTable("probabilities", "bx", object = CoCo.Object);
showTable("probabilities", "cx", object = CoCo.Object);
